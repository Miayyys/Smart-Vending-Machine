...
// NOTE: On OV5647 register 0x3820 is *horizontal* (bit1 = mirror) and
// 0x3821 is *vertical* (bit1 = flip). The previous implementation wrote the
// opposite registers, so setting VFLIP actually toggled mirror and vice-versa.
// Because mirror/flip also permute the Bayer phase, this mismatch produced a
// 1-pixel Bayer phase offset -> red rendered as green. Fix by writing the
// correct register for each operation.
