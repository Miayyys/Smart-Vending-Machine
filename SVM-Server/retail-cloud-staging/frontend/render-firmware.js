...
(function () {
        ...
        '<div class="fw-top">' +
          ...
          '<div class="ctrl-panel" style="flex:1">' +
            ...
            '<div class="form-group"><label>文件</label>' +
              '<div class="file-upload"><input type="file" id="fw-file" accept=".bin,.hex,.elf"><label for="fw-file" class="file-label">选择固件文件</label></div>' +
            '</div>' +
    ...
    API.getFirmwareList().then(function (list) {
      ...
      if (sel) {
        sel.innerHTML = '<option value="">-- 选择固件版本 --</option>' +
          list.map(function (f) { return '<option value="' + f.id + '">' + f.version + ' (' + formatSize(f.fileSize) + ')</option>'; }).join('');
      ...
      el.innerHTML =
        '<table><thead><tr><th>ID</th><th>版本</th><th>文件名</th><th>大小</th><th>MD5</th><th>描述</th><th>上传时间</th><th>操作</th></tr></thead><tbody>' +
        list.map(function (f) {
          return '<tr>' +
            '<td>' + f.id + '</td>' +
  ...
  function doUpload() {
    ...
    API.uploadFirmware(file, version, desc).then(function () {
