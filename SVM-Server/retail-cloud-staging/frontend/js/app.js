...
(function () {
  ...
  App._intervals = [];
  App._mqttOff = [];
  App._anomalyLog = JSON.parse(localStorage.getItem('anomaly_log') || '[]');
  ...
  function boot() {
    ...
    initProductMap().then(function () {
      ...
      // 全局异常事件监听（不受页面切换影响）
      var onAnomalyEvent = function (topic, data) {
        if (!data) return;
        var parts = topic.split('/');
        var dev = parts[1] || '?';
        var isDevice = topic.includes('/event/anomaly');
        App._anomalyLog.unshift({
          time: new Date().toLocaleString(),
          source: isDevice ? '设备上报' : '服务端检测',
          device: dev,
        ...
        localStorage.setItem('anomaly_log', JSON.stringify(App._anomalyLog));
      };
      App.MQTT.on('retail/+/event/anomaly', onAnomalyEvent);
      App.MQTT.on('retail/+/msg/err', onAnomalyEvent);
